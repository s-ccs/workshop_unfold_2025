...
question_box(md"""Try changing one of the values below. Pluto automatically updates all dependant cells! How does the resulting EEG change?
""")
...
eegdata,events = UnfoldSim.predef_eeg(;n_repeats,overlap=(min_overlap,0.2));

# ╔═╡ f854a1e7-3f37-4f38-9637-ea4326d6352d
(;size_eeg=size(eegdata),n_repeats,min_overlap) # just to display changing parameters
...
md"""
## Sliders
We can alsouse Sliders instead of fixing the parameters. 
...
# ╔═╡ ac0195ec-25a8-405a-9574-53d39e423cad
# @bind n_repeats PlutoUI.Slider(1:10,show_value=true)
# @bind min_overlap PlutoUI.Slider(0.1:0.05:0.5,show_value=true,default=0.5)

# ╔═╡ 0b009035-d91a-4c46-a762-3ae33e5bae18
question_box(md"""
Now it is your turn
1. Replace the `n_repeats` with a slider!
""")
...
md"""
### Experimental Design
We already started by simulating a design with a 2-level factor `condition` (🚗 vs. 😊) and one `continuous` effect from -5:5 (centered 👀 sacccade amplitude)
"""
...
md"""
## Epoching
The data are still continuous, so we need to epoch them
"""

# ╔═╡ 6bd3bf55-947a-43d9-a367-e816a1c2d9c9
eegdata_epochs, times = Unfold.epoch(data = eegdata, tbl = events, τ = (-0.4, 0.8), sfreq = 100); # channel x timesteps x trials
...
question_box(md"""
Go ahead, define a formula for the intercept and for the `condition`.	
...
m_erp = ismissing(f) ? still_missing() :
fit(UnfoldModel,[Any=>(f,times)],events,eegdata_epochs)
...
md"""
Two main ways to do it: Extract as a matrix (`coef`), or extract as a 🧹tidy dataframe (`coeftable`)
"""
...
# ╔═╡ 11ed8966-32c2-4f99-b944-2afd9e79f031
var_not_defined(:coefs)
...
manifest_format = "2.0"
project_hash = "6ac6c15ff5b690a2c52da9e4936464c0d49202ce"
...
git-tree-sha1 = "297b5b18be8610ab86a1c76895bee9d8d020725f"
...
git-tree-sha1 = "a5ba5bbfea32c3a7e33e5df5182621bd2abc990b"
