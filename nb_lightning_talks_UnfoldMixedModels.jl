...
# ╔═╡ b3ac7b4f-4869-43f1-8870-09675a298db6
...
|fixed effect||
|---|---|
|condition|$(@bind β_cond PlutoUI.Slider(0:4,show_value=true))|


|random effect||
|---|---|
|condition\|subject |$(@bind s_cond PlutoUI.Slider(1:4,show_value=true))|
|condition\|item |$(@bind i_cond PlutoUI.Slider(1:4,show_value=true))|
"""
...
# ╔═╡ d71b7260-21eb-4a4f-939f-039d310b53d1
	...
	d0 = factorproduct((;item=nlevels(nitem, "I")),(;subject=nlevels(nsub, "S")),(;condition=nlevels(2,"C")),(;trials=1:ntrials))|>DataFrame
	d0.y .= rand(size(d0,1)) # dummy response

	form_full = @formula(y~1+condition+(1+condition|subject) + (1+condition|item))
	m0 = fit(MixedModel, form_full, d0,contrasts=Dict(:condition=>EffectsCoding()))
	d_sim = deepcopy(d0)
	update!(m0; subject=create_re(1,s_cond),item=create_re(1,i_cond))
	
	simulate!(MersenneTwister(2),m0,σ=1,β=[0.,β_cond])
	d_sim.y = m0.y		
	m_analysis = fit(MixedModel,formula_analysis,d_sim)
...
AoG.data(d_sim)*mapping(:condition,:y,color=:subject)*visual(Beeswarm,algorithm=QuasirandomJitter(jitter_width=15))|>draw;
...
md"""
## Item effects inflate type-I errors
...
# ╔═╡ a3907b8a-12dd-4633-bf8b-0f47436a35d3
let
	t = ggthemr(:fresh)
	t.fontsize = 15 # presentation fontsize
	set_theme!(t)
end

# ╔═╡ 04f0b361-208e-411c-8a48-c0364712c741
...
MakieThemes = "~0.1.4"
MixedModels = "~4.34.1"
MixedModelsPermutations = "~0.2.0"
MixedModelsSim = "~0.2.11"
PlutoTeachingTools = "~0.3.1"
PlutoUI = "~0.7.62"
SwarmMakie = "~0.1.5"
Unfold = "~0.8.4"
UnfoldMakie = "~0.5.18"
UnfoldMixedModels = "~0.1.1"
UnfoldSim = "~0.4.0"
UnfoldStats = "~0.2.1"
"""
...
manifest_format = "2.0"
project_hash = "2a816c6461f0656a65680061b7d40a3c0b9f79eb"
...
[[deps.ClusterDepth]]
deps = ["Random", "SparseArrays", "StatsBase"]
git-tree-sha1 = "755d85215543b3d9ea215c6ed615fe713b66a7ed"
...
git-tree-sha1 = "efc2b5aee9b33a61abe27b72f0a7f48d6ce359a8"
...
git-tree-sha1 = "f5518aa8576390128b650c0408f3a27beee232d9"
...
[[deps.UnfoldMixedModels]]
deps = ["DataFrames", "DocStringExtensions", "LinearAlgebra", "MixedModels", "ProgressMeter", "Reexport", "SimpleTraits", "SparseArrays", "StaticArrays", "StatsAPI", "StatsModels", "Unfold"]
git-tree-sha1 = "4e72acfdfe056dd746dc3f6471e4f096ef620939"
...
version = "0.2.1"
weakdeps = ["ClusterDepth", "MixedModelsPermutations", "UnfoldMixedModels"]
