...
begin 
	...
	significancevalues = DataFrame(
	    from = [0.01, 0.2, 0.25],
	    to = [0.3, 0.4, 0.3],
	    coefname = ["(Intercept)", "condition: face", "continuous"], 
	)
	nothing
end
...
begin
    ...
    # 2. Markers as Arrows
    random_values = rand(64) .* 2π
    plot_topoplot!(t2, dat_raw[:, 50, 1]; positions,
                   axis = (; xlabel = "50 ms"),
                   topo_attributes = (;
                    label_scatter = (;
                        markersize = 20,
                        marker = '↑',
                        color = :black,
                        rotation = random_values,
                    )))

    # 3. Markersize change

    plot_topoplot!(t3, dat_raw[:, 50, 1]; positions,
                   axis = (; xlabel = "50 ms"),
                   topo_attributes = (;
                    label_scatter = (
                        markersize = random_values * 5,
                        marker = :circle,
                        color = :white,
                        strokecolor = :tomato
                    )))

    # 4. Colormap and Horizontal Colorbar
    plot_topoplot!(t4, dat_raw[:, 50, 1]; positions,
                   axis = (; xlabel = "50 ms"),
...
git-tree-sha1 = "2091707d9888b606cbc21ca66ee27517e2f301aa"
...
git-tree-sha1 = "cb67e723d34ec289c51be7386fc8e486937a1a12"
